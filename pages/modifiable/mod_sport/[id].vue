...
const sport = await useFetch('/api/sport/all_sport_info', {
    query: {
        sport_id: parseInt(route.params.id)
    }
})
...
const deleteResponsable = async () => {
    const {data, error} = await useFetch('/api/protected_action/delete_responsable', {
        method: 'POST',
        body: {
...
const closeAddModal = () => {
    ...
    newResponsable.value = { name: '', email: '', image: '' };
};

const addResponsable = async () => {
    const {data, error} = await useFetch('/api/protected_action/add_responsable', {
        method: 'POST',
        body: {
            sport_id: valsport.id,
            username: newResponsable.value.username,
            password: newResponsable.value.password,
            name: newResponsable.value.name,
            email: newResponsable.value.email,
            image: newResponsable.value.image
        }
    })
    closeAddModal();
    window.location.reload();
...
<template>
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="text-center mb-8">
            <h1 class="text-4xl font-bold text-purple-800 mb-2">Gestion de sport</h1>
            <p class="text-lg text-gray-700">{{ valsport.name }} à {{ valsport.city }}</p>
        </div>

        <!-- Responsables -->
        <div>
            <h2 class="text-2xl font-semibold text-purple-700 mb-4">Responsables:</h2>
            <div class="mb-8">
                <button @click="showAddForm" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">
                    Ajouter Responsable
                </button>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <div v-for="item in valsport.user" :key="item.id"
                    class="bg-white shadow-lg rounded-lg p-6 flex flex-col items-center transform transition duration-300 ease-in-out hover:scale-105">
                    <!-- <img v-bind:src="item.image" alt="Photo du responsable" class="w-24 h-24 rounded-full mb-4 object-cover"> -->
            ...
            <div v-if="showModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                <div class="bg-white p-8 rounded-lg shadow-lg text-center">
                    ...
                    <p class="mb-6">Êtes-vous sûr de vouloir supprimer ce responsable ?</p>
                    <div class="flex justify-center">
                        <button @click="deleteResponsable"
            ...
            <div v-if="showAddModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                <div class="bg-white p-8 rounded-lg shadow-lg text-center">
                    <h3 class="text-xl font-semibold mb-4">Ajouter un nouveau responsable</h3>
                    <form @submit.prevent="addResponsable">
                        <div class="mb-4">
                            <input v-model="newResponsable.username" type="text" placeholder="Username"
                                class="w-full p-2 border border-gray-300 rounded">
                        </div>
                        <div class="mb-4">
                            <input v-model="newResponsable.password" type="text" placeholder="Password"
                                class="w-full p-2 border border-gray-300 rounded">
                        </div>
                        <div class="mb-4">
                            <input v-model="newResponsable.name" type="text" placeholder="Nom complet"
                                class="w-full p-2 border border-gray-300 rounded">
                        </div>
                        <div class="mb-4">
                            <input v-model="newResponsable.email" type="email" placeholder="Email"
                                class="w-full p-2 border border-gray-300 rounded">
                        </div>
                        <div class="mb-4">
                            <input v-model="newResponsable.image" type="text" placeholder="URL de l'image"
                                class="w-full p-2 border border-gray-300 rounded">
                        </div>
                        <div class="flex justify-center">
                            <button type="submit"
                    ...
                    <form class="space-y-4" @submit.prevent="modifierHoraire(index)">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                id="horaire-date" type="text" v-model="horaire.date" placeholder="Entrez le titre">
                        </div>
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-2" for="news-title">
                                Heures
                            ...
                            <input
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="horaire-heure" type="text" v-model="horaire.time" placeholder="Entrez le titre">
                        </div>
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-2" for="news-title">
                                Lieu
                            ...
                            <input
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="horaire-lieu" type="text" v-model="horaire.lieu" placeholder="Entrez le titre">
                        </div>
                        <button type="submit"
                            class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Enregistrer</button>
                        <button type="button" @click="cancelEditHoraire(index)"
                            class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Annuler</button>
                    </form>
                </div>
                <button @click="deleteHoraire(index)"
                    class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded mt-4">Supprimer</button>
            </div>
            <div class="bg-white shadow-lg rounded-lg p-6 mb-6">
                <div v-if="!isAddingHoraires">
                    <button @click="isAddingHoraires = true"
                        class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Ajouter</button>
                </div>
                <div v-else>
                    <form @submit.prevent="ajoutHoraire" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="new-horaires-date" type="text" v-model="newHoraire.date"
                        ...
                        <div>
                            ...
                            <input
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="new-horaires-heures" type="text" v-model="newHoraire.time"
                                placeholder="Entrez le heure">
                        </div>
                        <div>
                            ...
                            <input
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="new-news-lieu" type="text" v-model="newHoraire.lieu" placeholder="Entrez le lieu">
                        </div>
                        <button type="submit"
                            class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Enregistrer</button>
                        <button @click="isAddingHoraires = false"
                            class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Annuler</button>
                    </form>
                </div>
            </div>
        ...
        <div>
            <div v-for="(news, index) in actualites" :key="index" class="bg-white shadow-lg rounded-lg p-6 mb-6">
                <div v-if="!news.isEditing">
                    ...
                    <button @click="editNews(index)"
                        class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Modifier</button>
                </div>
                <div v-else>
                    <form class="space-y-4" @submit.prevent="modifierActu(index)">
                        <!-- Form fields for editing news -->
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-2" for="news-title">
                                Titre
                            ...
                            <input
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="news-title" type="text" v-model="news.title" placeholder="Entrez le titre">
                        </div>
                        <div>
                            ...
                            <textarea
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="news-description" v-model="news.description"
                        ...
                        <div>
                            ...
                            <input
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="news-image" type="text" v-model="news.image" placeholder="Entrez l'URL de l'image">
                        </div>
                        <button type="submit"
                            class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Enregistrer</button>
                        <button type="button" @click="cancelEditNews(index)"
                            class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Annuler</button>
                    </form>
                </div>
                <button @click="deleteNews(index)"
                    class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded mt-4">Supprimer</button>
            </div>

            <div class="bg-white shadow-lg rounded-lg p-6 mb-6">
                <div v-if="!isAddingNews">
                    <button @click="isAddingNews = true"
                        class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Ajouter</button>
                </div>
                <div v-else>
                    <form @submit.prevent="ajoutActu" class="space-y-4">
                        <!-- Form fields for adding new news -->
                        <div>
                            ...
                            <input
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="new-news-title" type="text" v-model="newNews.titre" placeholder="Entrez le titre">
                        </div>
                        <div>
                            ...
                            <textarea
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="new-news-description" v-model="newNews.description"
                                placeholder="Entrez la description"></textarea>
                        </div>
                        <div>
                            ...
                            <input
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="new-news-image" type="text" v-model="newNews.image"
                                placeholder="Entrez l'URL de l'image">
                        </div>
                        <button type="submit"
                            class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Enregistrer</button>
                        <button @click="isAddingNews = false"
                            class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Annuler</button>
                    </form>
                </div>
            </div>
    ...
    <section class="mb-12">
        ...
        <div>
            <div v-for="(event, index) in events" :key="index" class="bg-white shadow-lg rounded-lg p-6 mb-6">
                ...
                <div v-else>
                    <form class="space-y-4" @submit.prevent="modifierEvenement(index)">
                        ...
                        <div>
                            ...
                            <input
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="event-title" type="text" v-model="event.title" placeholder="Entrez le titre">
                        </div>
                        <div>
                            ...
                            <textarea
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="event-plan" v-model="event.plan" placeholder="Entrez le plan"></textarea>
                        </div>
                        <button type="submit"
                            class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Enregistrer</button>
                        <button @click="cancelEditEvent(index)"
                            class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Annuler</button>
                    </form>
                </div>
            ...
            <div class="bg-white shadow-lg rounded-lg p-6 mb-6">
                <div v-if="!isAddingEvent">
                    <button @click="isAddingEvent = true"
                        class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Ajouter</button>
                </div>
                <div v-else>
                    <form class="space-y-4" @submit.prevent="adjoutEvenement">
                        ...
                        <div>
                            ...
                            <input
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="new-event-title" type="text" v-model="newEvent.titre" placeholder="Entrez le titre">
                        </div>
                        <div>
                            ...
                            <textarea
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="new-event-plan" v-model="newEvent.plan" placeholder="Entrez le plan"></textarea>
                        </div>
                        <button type="submit"
                            class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Enregistrer</button>
                        <button @click="isAddingEvent = false"
                            class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Annuler</button>
                    </form>
                </div>
            </div>
